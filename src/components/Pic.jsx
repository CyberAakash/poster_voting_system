  ...
  return (
    <div className="relative flex flex-row rounded-2xl items-start justify-start min-h-[13rem] min-w-[20rem] bg-white text-black border-2 border-black">
      ...
      <div className="flex flex-col text-xl font-semibold gap-6 items-center justify-center w-full min-h-full">
        <h1 className="border-b-2 border-black w-full text-center p-2 text-base sm:text-xl lg:text-3xl font-extralight font-mono">{name}  [{cardNo}]</h1>
        <h1 className="">{author}</h1>
        <p>{desc}</p>
        <div className="absolute text-center z-9 right-0 bottom-0 bg-greenBlue rounded-br-xl min-w-[50%] text-white text-sm p-4">Count : {vote_count}</div>
      </div>
